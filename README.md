...
- [ ] #_Загрузка константы_ записывает в память с адресом(adress)
    const: {
    value: 10,
    adress: 1
    }
- [ ] #_Чтение значения из памяти_ записывает в регистр-аккумулятор
    read: {
    adress: 1
    }
- [ ] #Запись значения в память_ записывает из регистра-аккумулятора
    write: {
    adress: 1
    }
- [ ] #_Унарная операция: bswap()_ берёт значение из регистра-аккумулятора
    bswap: {
    }
